...
define(['N/log', 'N/search', 'N/record', 'N/runtime', 'N/task'], (log, search, record, runtime, task) => {
    ...
    const ER_EXPENSE_LINES_SEARCH = 'customsearch_co_expense_report_exp_lines'; //CO Expense Report Expense Lines - CP PRODUCCION
    ...
    const beforeLoad = (context) => {
        ...
        if (context.type === context.UserEventType.VIEW) {
            // let isClosed = context.newRecord.getValue({ fieldId: 'status' });

            let statusRef = context.newRecord.getValue({ fieldId: 'statusRef' });
            let purchaseOrder = context.newRecord.getValue({ fieldId: 'tranid' });
            let isClosed = context.newRecord.getValue({ fieldId: 'custbody_lh_isclosed' });
            let voided = context.newRecord.getValue({ fieldId: 'voided' });
            log.debug('Verify-OC', 'INIT =====================');
            log.debug('purchaseOrder', purchaseOrder);
            log.debug('isClosed', isClosed);
            log.debug('statusRef', statusRef);
            log.debug('voided', voided);
            log.debug('Verify-OC', 'END ======================');
            if (statusRef == CLOSE_STATUS && isClosed == false && voided == "F") {
                log.debug('INICIO', 'INICIO PURCHASE_ORDER CLOSE ====================================');
                ...
                if (estadoPPTO == COMPROMETIDO || estadoPPTO == EJECUTADO || estadoPPTO == PAGADO) {
                    try {
                        let year = objRecord.getValue({ fieldId: 'custbody_lh_anio_id_flag' });
                        if (year.length != 0) {
                            log.debug('Close', 'Entré a cerrar');
                            let date = objRecord.getValue({ fieldId: 'trandate' });
                            date = sysDate(date); //! sysDate (FUNCTION)
                            let month = date.month;
                            let objSearch = search.load({ id: DETALLE_TRANSACCION_SEARCH });
                            ...
                            for (let i in result) {
                                ...
                                if (exeReverse == 1) {
                                    let featureRecord = record.submitFields({ type: DETALLE_TRANSACCION_RECORD, id: internalid, values: { 'custrecord_lh_cp_dt_comprometido': 0 } });
                                    log.debug('Close', 'Detalle: ' + featureRecord + ' closed');
                                }
                            }
                        }
                    } catch (error) {
                ...
                record.submitFields({ type: record.Type.PURCHASE_ORDER, id: context.newRecord.id, values: { 'custbody_lh_isclosed': true } });
                log.debug('FIN', 'FIN PURCHASE_ORDER CLOSE =======================================');
            } else {
                //log.debug('Verify-Closed', 'Ya estoy cerrado');
            }
        }
    }
    ...
    const beforeSubmit = (context) => {
        log.debug('Event-beforeSubmit', context.type);
        if (context.type === context.UserEventType.EDIT) {
            ...
            if (objRecord.type == PURCHASE_ORDER) {
                ...
                if (isJEVoid) {
                    ...
                    if (objRecord.getValue('custbody_lh_update_flag') != 5) {
                        let reset = 0;
                        let estadoPPTO = objRecord.getValue({ fieldId: 'custbody_lh_cp_estado_ppto_oc' });
                        ...
                        if (estadoPPTO == RESERVADO && updateFlag == 1) {
                            ...
                            log.debug('INICIO', 'INICIO PURCHASE_ORDER EDIT BEFORE ====================================');
                            //?BLOQUE DE AJUSTE Y CREACION ===================================================
                            let editCategories = JSON.parse(objRecord.getValue({ fieldId: 'custbody_lh_categories_id_edit_flag' }));
                            //*Params =======================
                            let year = objRecord.getValue({ fieldId: 'custbody_lh_anio_id_flag' });
                            let date = objRecord.getValue({ fieldId: 'trandate' });
                            date = sysDate(date); //! sysDate (FUNCTION)
                            ...
                            //*=============================
                            for (let j in editCategories) {
                                let ceco = editCategories[j][2];
                                let category = editCategories[j][0];
                                let thisAmount = editCategories[j][1];
                                thisAmount = parseFloat(thisAmount.toFixed(2));
                                log.debug('CATEGORIA=======', category);
                                log.debug('THIS_AMOUNT=======', thisAmount);
                                let objSearch = search.load({ id: DETALLE_TRANSACCION_SEARCH });
                                let filters = objSearch.filters;
                                const filterOne = search.createFilter({ name: 'custrecord_lh_cp_dt_purchase_ord_related', operator: search.Operator.ANYOF, values: objRecord.id });
                                filters.push(filterOne);
                                const filterTwo = search.createFilter({ name: 'custrecord_lh_cp_dt_centro_costo', operator: search.Operator.ANYOF, values: ceco });
                                filters.push(filterTwo);
                                let countRes = objSearch.runPaged().count;
                                if (countRes != 0) {
                                    log.debug('Ajuste', category);
                                    ...
                                    let categoria = result[0].getValue({ name: "custrecord_lh_cp_dt_category_ppto" });
                                    let comprometido = parseFloat(result[0].getValue({ name: "custrecord_lh_cp_dt_comprometido" }));
                                    let ejecutado = parseFloat(result[0].getValue({ name: "custrecord_lh_cp_dt_ejecutado" }));
                                    let pagado = parseFloat(result[0].getValue({ name: "custrecord_lh_cp_dt_pagado" }));
                                    let disponible = parseFloat(result[0].getValue({ name: "custrecord_lh_cp_dt_disponible" }));

                                    if (thisAmount != (comprometido + ejecutado + pagado + disponible)) {
                                        let ajuste = thisAmount - (comprometido + ejecutado + pagado + disponible);
                                        //! 250 = 500-750
                                        // if (ajuste < 0 && ejecutado == 0 && pagado == 0) { } //! <=== Validar si se tomará en cuenta, punto muy abierto
                                        if (comprometido < thisAmount && ejecutado == 0 && pagado == 0) {
                                            record.submitFields({
                                                type: DETALLE_TRANSACCION_RECORD,
                                                id: internalid,
                                                values: {
                                                    'custrecord_lh_cp_dt_reservado': Math.abs(thisAmount),
                                                    'custrecord_lh_cp_dt_comprometido': 0.00,
                                                    'custrecord_lh_cp_dt_estado_busqueda': RESERVADO,
                                                    'custrecord_lh_cp_dt_accion_ppto': RESERVADO
                                                }
                                            });
                                            ajustePPTO(categoria, year, month, ajuste); //!Ajusta ppto ya sea mayor o menor
                                            updatePresupuesto(month, year, categoria, ajuste) //!Ajusta ppto ya sea mayor o menor
                                            reset = 1;
                                        } else if (comprometido >= ajuste || ejecutado > 0 || pagado > 0) {
                                            //! 250 = 500-750
                                            // if (comprometido < editCategories[j][1] && ejecutado == 0 && pagado == 0) {
                                            //     record.submitFields({
                                            //         type: DETALLE_TRANSACCION_RECORD,
                                            //         id: internalid,
                                            //         values: {
                                            //             'custrecord_lh_cp_dt_reservado': Math.abs(editCategories[j][1]),
                                            //             'custrecord_lh_cp_dt_comprometido': 0.00,
                                            //             'custrecord_lh_cp_dt_estado_busqueda': RESERVADO,
                                            //             'custrecord_lh_cp_dt_accion_ppto': RESERVADO
                                            //         }
                                            ...
                                            //     reset = 1;
                                            // } else {
                                            log.debug('ENTRE-1', ajuste);
                                            record.submitFields({ type: DETALLE_TRANSACCION_RECORD, id: internalid, values: { 'custrecord_lh_cp_dt_comprometido': comprometido + ajuste } });
                                            ajustePPTO(categoria, year, month, ajuste); //!Ajusta ppto ya sea mayor o menor
                                            // }
                                        } else {
                                            log.debug('ENTRE-2', ajuste);
                                            // let ajuste = editCategories[j][1] - (comprometido + ejecutado + pagado);
                                            // if (comprometido > ajuste) {
                                            ...
                                            //         id: internalid,
                                            //         values: {
                                            //             'custrecord_lh_cp_dt_reservado': ajuste,
                                            //             'custrecord_lh_cp_dt_comprometido': 0.00,
                                            //             'custrecord_lh_cp_dt_estado_busqueda': RESERVADO,
                                            //             'custrecord_lh_cp_dt_accion_ppto': RESERVADO
                                            //         }
                                            ...
                                            record.submitFields({
                                                ...
                                                id: internalid,
                                                values: {
                                                    'custrecord_lh_cp_dt_reservado': Math.abs(ajuste),
                                                    'custrecord_lh_cp_dt_comprometido': 0.00,
                                                    'custrecord_lh_cp_dt_estado_busqueda': RESERVADO,
                                                    'custrecord_lh_cp_dt_accion_ppto': RESERVADO
                                                }
                                    ...
                                    arrayCategorias.push([internalid.toString(), thisAmount]);
                                    log.debug('Detalle-Transacción', internalid);
                                } else {
                                    log.debug('Nuevo', ceco); 
                                    let tipoCambio = '';
                                    ...
                                    let transaction = objRecord.getValue({ fieldId: 'custbody_lh_transaction_type_flag' });
                                    let date = objRecord.getValue({ fieldId: 'trandate' });
                                    date = sysDate(date); //! sysDate (FUNCTION)
                                    let month = date.month;
                                    let internalidPeriod = getPeriod(month, date.year); //! FUNCTION => getPeriod
                                    if (currency != CURRENCY_US_DOLLAR && currency != CURRENCY_CANADIAN_DOLLAR) {
                                        tipoCambio = getTipoCambio2(subsidiary, month, date.year); //! getTipoCambio (FUNCTION)
                                        if (tipoCambio == 0) {
                                            dialog.alert({ title: 'Información', message: 'No se encuentra un tipo de cambio para esta transacción' });
                                            return false;
                                        }
                                    }
                                    let amount = thisAmount;
                                    let criteria = editCategories[j][2];
                                    let response = getPresupuesto(category, year, month, temporalidad, objRecord.id, criteria, subsidiary, criteria, amount, nivelControl, transaction, internalidPeriod, tipoCambio, 4); //! FUNCTION => getPresupuesto
                                    arrayCategorias.push([response.toString(), amount]);
                                    log.debug('Detalle-Transacción', response);
                                    reset = 1;
                                }
                            }
                            objRecord.setValue('custbody_lh_update_flag', '');

                            //?BLOQUE DE ELIMINACION =========================================================
                            log.debug('ELIMINACION', 'BLOQUE====');
                            let flagCategories = JSON.parse(objRecord.getValue({ fieldId: 'custbody_lh_categories_id_flag' }));//!COMENTAR PARA CUADRE
                            log.debug('ARRAYNOW', flagCategories);//!COMENTAR PARA CUADRE
                            log.debug('ARRAYTHIS', arrayCategorias);
                            //!COMENTAR PARA CUADRE
                            for (let k in flagCategories) {
                                let notExist = 0;
                                let detalleid = flagCategories[k][0];
                                for (let m = 0; m < arrayCategorias.length; m++) {
                                    if (flagCategories[k][0] == arrayCategorias[m][0]) {
                                        notExist = 1;
                                        break;
                                    }
                                }
                                log.debug('EXIST', 'Detalle: ' + detalleid + ' - exist?: ' + notExist);
                                if (notExist == 0) {
                                    let objSearch = search.load({ id: DETALLE_TRANSACCION_SEARCH });
                                    let filters = objSearch.filters;
                                    const filterOne = search.createFilter({ name: 'internalid', operator: search.Operator.ANYOF, values: detalleid });
                                    filters.push(filterOne);
                                    let countRes = objSearch.runPaged().count;
                                    if (countRes > 0) {
                                        let result = objSearch.run().getRange({ start: 0, end: 1 });
                                        let ajusteRes = parseFloat(result[0].getValue({ name: "custrecord_lh_cp_dt_reservado" }));
                                        let eje = parseFloat(result[0].getValue({ name: "custrecord_lh_cp_dt_ejecutado" }));
                                        let pag = parseFloat(result[0].getValue({ name: "custrecord_lh_cp_dt_pagado" }));
                                        let internalid = result[0].getValue({ name: "internalId" });
                                        let categoria = result[0].getValue({ name: "custrecord_lh_cp_dt_category_ppto" });
                                        // let ajusteCom = parseFloat(result[0].getValue({ name: "custrecord_lh_cp_dt_comprometido" }));
                                        if (ajusteRes > 0) {
                                            let exeReverse = executeReverseReservado(categoria, year, month, -ajusteRes); //!Ajusta ppto ya sea mayor o menor
                                            if (exeReverse == 1) {
                                                let featureRecord = record.delete({ type: DETALLE_TRANSACCION_RECORD, id: internalid });
                                                log.debug('Delete', 'Detalle: ' + featureRecord + ' delete');
                                            }
                                            ...
                                            if (eje == 0 && pag == 0) {
                                                let ajuste = parseFloat(result[0].getValue({ name: "custrecord_lh_cp_dt_comprometido" }));
                                                let exeReverse = ajustePPTO(categoria, year, month, -ajuste); //!Ajusta ppto ya sea mayor o menor
                                                if (exeReverse == 1) {
                                                    let featureRecord = record.delete({ type: DETALLE_TRANSACCION_RECORD, id: internalid });
                                                    log.debug('Delete', 'Detalle: ' + featureRecord + ' delete');
                                                }
                            ...
                            //?BLOQUE RESET =================================================================
                            if (reset == 1) {
                                objRecord.setValue('approvalstatus', reset);
                            }
                            log.debug('FIN', 'FIN PURCHASE_ORDER EDIT BEFORE ====================================');
                        }
                    }
            ...
            if (objRecord.type == VENDOR_BILL) {
                ...
                } else if (context.newRecord.getValue({ fieldId: 'custbody_lh_update_flag' }) != 7) {
                    let monto = 0;
                    let estadoPPTO = objRecord.getValue({ fieldId: 'custbody_lh_cp_estado_ppto_oc' });
                    let estadoPPTOTXT = objRecord.getText({ fieldId: 'custbody_lh_cp_estado_ppto_oc' });
                    let updateFlag = objRecord.getValue({ fieldId: 'custbody_lh_update_flag' });
                    let createdfrom = objRecord.getValue({ fieldId: 'custbody_lh_create_from_flag' });
                    let idBill = objRecord.getValue({ fieldId: 'id' });
                    log.debug('ExecuteAction', estadoPPTOTXT + ' - ' + updateFlag);
            ...
            if (objRecord.type == EXPENSE_REPORT) {
                if (objRecord.getValue({ fieldId: 'custbody_lh_update_flag' }) != 7) {
                    let isJEVoid = context.newRecord.getValue({ fieldId: 'void' });
                    log.debug("beforeSubmit-void", isJEVoid);
                    if (isJEVoid) {
                        try {
                        ...
                        if (estadoPPTO == RESERVADO && updateFlag == 1) {
                            log.debug('INICIO', 'INICIO EXPENSE_REPORT EDIT BEFORE ====================================');
                            try {
                                let year = objRecord.getValue({ fieldId: 'custbody_lh_anio_id_flag' });
                                if (year.length != 0) {
                                    log.debug('Delete', 'Entré a eliminar');
                            ...
                            log.debug('FIN', 'FIN EXPENSE_REPORT EDIT BEFORE ====================================');
                        }

                        if (estadoPPTO == EJECUTADO && updateFlag == 1 || estadoPPTO == PAGADO && updateFlag == 1) {
                            log.debug('INICIO', 'INICIO EXPENSE_REPORT EDIT BEFORE ====================================');
                            try {
                                let param = 0;
                                let year = objRecord.getValue({ fieldId: 'custbody_lh_anio_id_flag' });
                                if (year.length != 0) {
                                    ...
                                    for (let i in result) {
                                        ...
                                        if (estadoPPTO == EJECUTADO) {
                                            param = parseFloat(result[i].getValue({ name: "custrecord_lh_cp_dt_ejecutado" }));
                                        } else if (estadoPPTO == PAGADO) {
                                            param = parseFloat(result[i].getValue({ name: "custrecord_lh_cp_dt_pagado" }));
                                        }

                                        let category = result[i].getValue({ name: "custrecord_lh_cp_dt_category_ppto" });
                                        let exeReverse = executeReverseComprometido(category, year, month, param, EXPENSE_REPORT, 0, estadoPPTO);
                                        if (exeReverse == 1) {
                                            let featureRecord = record.delete({ type: DETALLE_TRANSACCION_RECORD, id: internalid });
                                            log.debug('Delete', 'Detalle: ' + featureRecord + ' delete');
                                        }
            ...
            if (objRecord.type == VENDOR_PAYMENT) {
                ...
                if (isJEVoid) {
                    try {
                        ...
                        let lookupFields1 = search.lookupFields({ type: search.Type.TRANSACTION, id: context.newRecord.id, columns: ['custbody_lh_categories_id_flag'] });
                        let categoriesArray = JSON.parse(lookupFields1.custbody_lh_categories_id_flag);
                        ...
                        for (let k in result) {
                            ...
                            for (let i in arrayCategorias) {
                                for (let m in categoriesArray) {
                                    let bloque = categoriesArray[m][1].includes(arrayCategorias[i][0]);
                                    if (bloque == true) {
                                        log.debug('Monto a devolver pago', categoriesArray[m][2] + ' - ' + arrayCategorias[i][0]);
                                        monto = categoriesArray[m][2];
                                    }
                                }
                                log.debug('Init-arrayCategorias', 'Detalle: ' + arrayCategorias[i][0] + ' - Monto: ' + monto);
                            ...
                            log.debug('newArrayCategorias', arrayCategorias);
                            if (pagadoFlag == 0) {
                                if (type == VENDOR_BILL_TYPE) {
                                    record.submitFields({
        ...
        //                 const filterOne = search.createFilter({ name: 'custrecord_lh_cp_dt_purchase_ord_related', operator: search.Operator.ANYOF, values: objRecord.id });
        //                 filters.push(filterOne);
        //                 const filterTwo = search.createFilter({ name: 'custrecord_lh_cp_dt_category_ppto', operator: search.Operator.ANYOF, values: category });
        //                 filters.push(filterTwo);
        //                 let countRes = objSearch.runPaged().count;
        //                 if (countRes != 0) {
        //                     log.debug('Ajuste', category);
        ...
        //                     if (editCategories[j][1] != (comprometido + ejecutado + pagado + disponible)) {
        //                         let ajuste = editCategories[j][1] - (comprometido + ejecutado + pagado + disponible);
        //                         //! 250 = 500-750
        //                         // if (ajuste < 0 && ejecutado == 0 && pagado == 0) { } //! <=== Validar si se tomará en cuenta, punto muy abierto
        //                         if (comprometido < editCategories[j][1] && ejecutado == 0 && pagado == 0) {
        //                             record.submitFields({
        ...
        //                         }
        //                     }
        //                     arrayCategorias.push([internalid.toString(), editCategories[j][1]]);
        //                     log.debug('Detalle-Transacción', internalid);
        //                 } else {
        //                     log.debug('Nuevo', category);
        //                     let tipoCambio = '';
        //                     let currency = objRecord.getValue({ fieldId: 'currency' });
        //                     let temporalidad = objRecord.getValue({ fieldId: 'custbody_lh_temporalidad_flag' });
        //                     let subsidiary = objRecord.getValue({ fieldId: 'subsidiary' });
        ...
        //             //?BLOQUE DE ELIMINACION =========================================================
        //             log.debug('ELIMINACION', 'BLOQUE====');
        //             //let flagCategories = JSON.parse(objRecord.getValue({ fieldId: 'custbody_lh_categories_id_flag' }));//!COMENTADO PARA CUADRE
        //             //log.debug('ARRAYNOW', flagCategories);//!COMENTADO PARA CUADRE
        //             log.debug('ARRAYTHIS', arrayCategorias);
        //             //!COMENTADO PARA CUADRE
        //             // for (let k in flagCategories) {
        //             //     let notExist = 0;
        //             //     let detalleid = flagCategories[k][0];
        //             //     for (let m = 0; m < arrayCategorias.length; m++) {
    ...
    const afterSubmit = (context) => {
        log.debug('Event-afterSubmit', context.type);
        const objRecord = context.newRecord;
                ...
                try {
                    ...
                    let objOC = search.lookupFields({
                        ...
                        columns: [
                            ...
                            'custbody_lh_transaction_type_flag'
                        ]
                    });
        ...
        if (context.type === context.UserEventType.EDIT) {
            ...
            if (objRecord.type == PURCHASE_ORDER) {
                if (objRecord.getValue('custbody_lh_update_flag') == 5) {
                    ...
                    try {
                        ...
                        //TODO Bloque para ingresar una oc al control presupuestal
                        log.debug('Saved-Edit', 'Orden de compra ' + recordId + ' ingresa al control presupuestal.');
                        executeCreated(recordId, objOC);
                        //record.submitFields({ type: record.Type.PURCHASE_ORDER, id: recordId, values: { 'custbody_lh_cp_estado_ppto_oc': 2 } });
                    } catch (error) {
                        log.error('Error-afterSubmit', error);
            ...
            //             let updateFlag = objOC.custbody_lh_update_flag;
            //             if (estadoPPTO == RESERVADO && updateFlag == 1) {
            //                 log.debug('Input', 'Entré a crear por update, no void');
            //                 executeCreated(recordId, objOC);
            //             } else if (estadoPPTO == RESERVADO && updateFlag == 2) { //TODO Bloque para ingresar una oc al control presupuestal
            //                 log.debug('Saved-Edit', 'Orden de compra ' + recordId + ' ingresa al control presupuestal.');
            //                 executeCreated(recordId, objOC);
            //             } else {
            //                 log.debug('Saved-Edit', 'Se guardó sin modificar o por ajuste de ppto');
            //             }
            //         } catch (error) {
            //             log.error('Error-afterSubmit', error);
            //             log.debug('ERROR', 'FIN PURCHASE_ORDER EDIT =======================================')
            ...
            if (objRecord.type == EXPENSE_REPORT) {
                let isJEVoid = context.newRecord.getValue({ fieldId: 'void' });
                if (!isJEVoid) {
                    log.debug('INICIO', 'INICIO EXPENSE_REPORT EDIT ====================================');
                    try {
                        let recordId = objRecord.id;
                        //record.submitFields({ type: search.Type.EXPENSE_REPORT, id: recordId, values: { 'custbody_lh_create_from_flag': recordId } });
                        // const type = context.type;
                        let objOC = search.lookupFields({
                            type: search.Type.EXPENSE_REPORT,
                            id: recordId,
                        ...
                        if (estadoPPTO == RESERVADO && updateFlag == 1) {
                            ...
                            log.debug('Saved-Edit', 'Informe de compra ' + recordId + ' ingresa al control presupuestal.');
                            executeCreated(recordId, objOC);
                        } else if (updateFlag == 7) {
                            executeCreated(recordId, objOC);
                            //record.submitFields({ type: record.Type.EXPENSE_REPORT, id: recordId, values: { 'custbody_lh_update_flag': '' } });
                        }
                        else {
                        ...
                        log.error('Error-afterSubmit', error);
                        log.debug('ERROR', 'FIN EXPENSE_REPORT EDIT =======================================')
                    }
                    log.debug('FIN', 'FIN EXPENSE_REPORT EDIT =======================================');
                }
            }
        }
    }
    ...
    const procesoPago = (id) => {
        try {
            ...
            log.debug('arrayPayDetails.length', arrayPayDetails.length);
            if (arrayPayDetails.length != 0) {
                arrayDeletePay = JSON.parse(arrayPayDetails);
                //arrayDeletePay = deletePay(recordId, JSON.parse(arrayPayDetails)); //!Revisar post prod
                //mappingPay(recordId);
            } else {
    ...
    const addPay = (recordId, arrayPayDetails) => {
        ...
        for (let i in result) {
            ...
            let suma = parseFloat(result[i].getValue({ name: "payingamount", join: "appliedToTransaction", summary: "GROUP" }));
            log.debug('SUMA', suma);
            if (suma > 0) {
                ...
                for (let j in result2) {
                    ...
                    arrayDetalle.push(internalId);
                    //!Existe
                    // for (let m in arrayPayDetails) {
                    //     let bloque = arrayPayDetails[k][1].includes(internalId);
                    //     if (bloque == true) {
                    //         monto = arrayPayDetails[k][1];
                    //     } 
                    // }
                    // arrayCategorias.map((dato) => {
                    //     if (dato[0] == arrayCategorias[i][0]) {
    ...
    const deletePay = (recordId, arrayPayDetails) => {
        log.debug('START', 'PAY DELETE INICIO ====================================');
        let param1 = 0;
        let param2 = 0;
        let statusEjecutado = '';
        let statusPagado = '';
            ...
            for (let i in arrayPayDetails) {
                ...
                for (let m in mappingpay) {
                    ...
                    if (exist == false) {
                        ...
                        let ejecutado = parseFloat(lookupFields2.custrecord_lh_cp_dt_ejecutado);
                        let pagorelacionado = lookupFields2.custrecord_lh_cp_dt_pago_relacionado;
                        ordenrelacionada = lookupFields2.custrecord_lh_cp_dt_purchase_ord_related;
                        log.debug('Look', typeof ordenrelacionada + ' - ' + ordenrelacionada.length);
                        if (ordenrelacionada.length > 0) {
                            ordenrelacionada = lookupFields2.custrecord_lh_cp_dt_purchase_ord_related[0].value;
                        }
                        pagado = pagado - param2;
                        ...
                        log.debug('Proceso-Anulación', 'Transacción ' + transactionid + ' con detalle: ' + arrayPayDetails[i][1] + ' pago anulado');
    ...
    const executeCreated = (recordId, objOC) => {
        ...
        if (temporalidad.length != 0 && subList.length != 0) {
            ...
            if (subList == ITEM_SUBLIST) {
                ...
                if (objOC.custbody_lh_transaction_type_flag == TRANSACTION_EXPENSE_REPORT) {
                    objSearch = search.load({ id: ER_EXPENSE_LINES_SEARCH });
                    join = 'expenseCategory';
                    transaction = TRANSACTION_EXPENSE_REPORT;
                } else {
                    objSearch = search.load({ id: PO_EXPENSE_LINES_SEARCH });
                    join = 'expenseCategory';
                    transaction = TRANSACTION_PURCHASE_ORDER;
            ...
            //* =====> INIT RESULTS 
            expenseCount = objSearch.runPaged().count;
            // if (objOC.custbody_lh_transaction_type_flag == TRANSACTION_EXPENSE_REPORT && expenseCount == 0) {
            //     objSearch = search.load({ id: ER_EXPENSE_LINES_SEARCH });
            //     filters = objSearch.filters;
            //     filterOne = search.createFilter({ name: 'internalid', operator: search.Operator.ANYOF, values: recordId });
            //     filters.push(filterOne);
            // }
            ...
            for (let i in results) {
                let amount = 0;
                let deparment = results[i].getValue(objSearch.columns[2]);
                log.debug('DEPARTAMENTO', deparment);
                //let subsidiary = results[i].getValue(objSearch.columns[3]);
                let account = results[i].getValue(objSearch.columns[4]);
                let category = results[i].getValue(objSearch.columns[5]);
            ...
            } else if (transaction == TRANSACTION_EXPENSE_REPORT) {
                record.submitFields({
                    type: record.Type.EXPENSE_REPORT,
                    id: recordId,
                    values: {
    ...
    const ajustePPTO = (category, year, month, monto) => {
        try {
            let saveRecord2 = 0;
            let saveRecord3 = 0;
            ...
            const filterThree = search.createFilter({ name: 'custrecord_lh_detalle_cppto_anio', operator: search.Operator.ANYOF, values: year });
            filters.push(filterThree);
            const filterTwo = search.createFilter({ name: 'custrecord_lh_detalle_cppto_status', operator: search.Operator.ANYOF, values: [EJECUTADO, COMPROMETIDO, DISPONIBLE, PAGADO] });
            filters.push(filterTwo);
            let result = objSearch.run().getRange({ start: 0, end: 20 });
            for (let i in result) {
                let lhStatus = result[i].getValue({ name: "custrecord_lh_detalle_cppto_status" });
                if (lhStatus == EJECUTADO) {
                    statusEjecutado = result[i].getValue({ name: "internalid" });
    ...
    const ajustePPTOReset = (category, year, month, monto) => {
        try {
            let saveRecord2 = 0;
            let saveRecord3 = 0;
            let statusEjecutado = 0;
            ...
            const filterThree = search.createFilter({ name: 'custrecord_lh_detalle_cppto_anio', operator: search.Operator.ANYOF, values: year });
            filters.push(filterThree);
            const filterTwo = search.createFilter({ name: 'custrecord_lh_detalle_cppto_status', operator: search.Operator.ANYOF, values: [EJECUTADO, COMPROMETIDO, RESERVADO, PAGADO] });
            filters.push(filterTwo);
            let result = objSearch.run().getRange({ start: 0, end: 20 });
            for (let i in result) {
                let lhStatus = result[i].getValue({ name: "custrecord_lh_detalle_cppto_status" });
    ...
    const executeReverseComprometido = (category, year, month, monto, transaction = 0, createdfrom = 0, estado = 0, montoAcual = 0, ejecutadoActual = 0, updateFlag = 0) => {
        try {
            ...
            if (transaction == VENDOR_BILL) {
            ...
            } else if (transaction == EXPENSE_REPORT) {
                if (estado == EJECUTADO) {
                    montoEjecutado = montoEjecutado - monto;
                    log.debug('Resta comprometido', montoEjecutado + ' = ' + ' ' + montoEjecutado + ' - ' + monto);
                    let openRecord2 = record.load({ type: CATEGORIA_PERIODO_RECORD, id: statusEjecutado, isDynamic: true });
                    ...
                    log.debug('Resta comprometido', montoPagado + ' = ' + ' ' + montoPagado + ' - ' + monto);
                    let openRecord2 = record.load({ type: CATEGORIA_PERIODO_RECORD, id: statusPagado, isDynamic: true });
                    openRecord2.setValue({ fieldId: "custrecord_lh_detalle_cppto_" + month, value: montoPagado });
                    saveRecord2 = openRecord2.save();
                ...
                if (estado == EJECUTADO || estado == PAGADO) {
                    if (updateFlag == 3) { //!Cuando es mayor
                        montoComprometido = montoComprometido + monto - montoAcual - ejecutadoActual;
                        log.debug('Resta comprometido', montoComprometido + ' = ' + ' ' + montoComprometido + ' + ' + monto + ' - ' + montoAcual + ' - ' + ejecutadoActual);
                        let openRecord2 = record.load({ type: CATEGORIA_PERIODO_RECORD, id: statusComprometido, isDynamic: true });
                        openRecord2.setValue({ fieldId: "custrecord_lh_detalle_cppto_" + month, value: montoComprometido });
                        saveRecord2 = openRecord2.save();

                        montoDisponible = montoDisponible - monto + montoAcual + ejecutadoActual;
                        log.debug('Devuelve disponible', montoDisponible + ' = ' + ' ' + montoDisponible + ' - ' + monto + ' + ' + montoAcual);
                        let openRecord3 = record.load({ type: CATEGORIA_PERIODO_RECORD, id: statusDisponible, isDynamic: true });
                        openRecord3.setValue({ fieldId: "custrecord_lh_detalle_cppto_" + month, value: montoDisponible });
                        saveRecord3 = openRecord3.save();
                    ...
                    } else if (updateFlag == 4) { //!Cuando es menor
                        montoComprometido = montoComprometido + monto - montoAcual - ejecutadoActual;
                        log.debug('Resta comprometido', montoComprometido + ' = ' + ' ' + montoComprometido + ' + ' + monto + ' - ' + montoAcual);
                        let openRecord2 = record.load({ type: CATEGORIA_PERIODO_RECORD, id: statusComprometido, isDynamic: true });
                        openRecord2.setValue({ fieldId: "custrecord_lh_detalle_cppto_" + month, value: montoComprometido });
                        saveRecord2 = openRecord2.save();
    ...
    const getPresupuesto = (category, year, month, temporalidad, recordId, deparment, subsidiary, account, amount_required, nivelControl, transaction, internalidPeriod, tipoCambio, paramCriteria = 0) => {
        let disponible = 0;
        ...
        try {
            ...
            if (transaction == TRANSACTION_PURCHASE_ORDER || transaction == TRANSACTION_EXPENSE_REPORT) {
                ...
                if (paramCriteria != 4) {
                    ...
                    objCreate.setValue({ fieldId: 'custrecord_lh_cp_dt_reservado', value: amount });
                    let response = objCreate.save();
                    updatePresupuesto(month, year, category, amount) //!Ajusta ppto ya sea mayor o menor
                    //updatePresupuestoComp(month, year, category, amount, temporalidad);
                    return response;
                }
    ...
    const updatePresupuestoComp = (month, year, category, amount, temporalidad) => {
        let statusComprometido = '';
        let statusDisponible = '';
        let montoComprometido = 0;
        let montoDisponible = 0;
        ...
        const filterThree = search.createFilter({ name: 'custrecord_lh_detalle_cppto_anio', operator: search.Operator.ANYOF, values: year });
        filters.push(filterThree);
        const filterTwo = search.createFilter({ name: 'custrecord_lh_detalle_cppto_status', operator: search.Operator.ANYOF, values: [DISPONIBLE, COMPROMETIDO] });
        filters.push(filterTwo);
        ...
        if (temporalidad == TEMPORALIDAD_MENSUAL || temporalidad == TEMPORALIDAD_TRIMESTRAL || temporalidad == TEMPORALIDAD_ANUAL) {
            ...
            let openRecord2 = record.load({ type: CATEGORIA_PERIODO_RECORD, id: statusDisponible, isDynamic: true });
            let openRecord3 = record.load({ type: CATEGORIA_PERIODO_RECORD, id: statusComprometido, isDynamic: true });
            openRecord2.setValue({ fieldId: "custrecord_lh_detalle_cppto_" + month, value: montoDisponible });
            openRecord3.setValue({ fieldId: "custrecord_lh_detalle_cppto_" + month, value: montoComprometido });
            let saveRecord2 = openRecord2.save();
            let saveRecord3 = openRecord3.save();
            log.debug('Recordsssss', saveRecord2 + ' - ' + saveRecord3 + ' - Categorías actualizadas');
            //!USER EVENT
            setTotalPpto(CATEGORIA_PERIODO_RECORD, saveRecord2);
            setTotalPpto(CATEGORIA_PERIODO_RECORD, saveRecord3);
        }
    ...
    const getTipoCambio2 = (subsidiary, month, year) => {
        ...
        log.debug('subsidiary-this', subsidiary);
        let loadSearch = search.load({ id: PERIODO_SEARCH });
        let filters2 = loadSearch.filters;
        const filterOne2 = search.createFilter({ name: 'periodname', operator: search.Operator.STARTSWITH, values: period });
        filters2.push(filterOne2);
