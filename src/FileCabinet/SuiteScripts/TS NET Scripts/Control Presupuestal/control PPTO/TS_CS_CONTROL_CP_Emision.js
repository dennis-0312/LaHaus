 ...
 */
 define(['N/currentRecord', 'N/search', 'N/ui/dialog', 'N/runtime'], (currentRecord, search, dialog, runtime) => {
    ...
    const pageInit = (context) => {
        ...
        if (typeMode == 'copy') {
            if (context.currentRecord.type != JOURNAL) {
                ...
                context.currentRecord.setValue('custbody_lh_cp_estado_ppto_oc', '');
            }
            ...
            context.currentRecord.setValue('custbody_lh_isclosed', false);
    ...
    const saveRecord = (context) => {
        const ambiente = 'SB';
        const DEVELOPER = ambiente; //PR - 27160 //JP - 27161
        const DEVELOPERJP = 27161;
        ...
        if (typeMode == CREATE || typeMode == COPY) {
            ...
            if (context.currentRecord.type == JOURNAL) {
                ...
                if (runtime.getCurrentUser().id == DEVELOPER || runtime.getCurrentUser().id == DEVELOPERJP) {
                    console.log('Pruebas internas');
                    return false;
                } else {
                    return retorno;
        ...
        if (typeMode == EDIT) {
            if (context.currentRecord.type == PURCHASE_ORDER) {
                // if (estado == RESERVADO || estado == COMPROMETIDO || estado == EJECUTADO || estado == PAGADO) {
                ...
                //     alert('El registro no puede editarse');
                //     return false;
                // }
                let retorno = '';
                if (context.currentRecord.getValue('custbody_lh_cp_estado_ppto_oc').length == 0) { //! Solo activar cuando falle creación de detalle de OC
                    // const objRecord = currentRecord.get();
                    // console.log('Saved-Edit', 'Orden de compra ' + objRecord.id + ' ingresa al control presupuestal.');
                    // retorno = controlPresupuestal(typeMode);
                    // context.currentRecord.setValue('custbody_lh_update_flag', 5);
                    // // context.currentRecord.setValue('approvalstatus', 1);
                    // context.currentRecord.setValue('custbody_lh_cp_estado_ppto_oc', 2);
                    return true;
                } else if (context.currentRecord.getValue('custbody_lh_update_flag') != 4) {
                    retorno = controlPresupuestal(typeMode);
                    // if (estado == RESERVADO) {
            ...
            if (context.currentRecord.type == EXPENSE_REPORT) {
                if (runtime.getCurrentUser().id == DEVELOPER) {
                    let retorno = controlPresupuestal();
                    context.currentRecord.setValue('custbody_lh_cp_estado_ppto_oc', PAGADO);
                    return retorno;
                } else {
            ...
            if (context.currentRecord.type == JOURNAL) {
                ...
                // if (estado.length == 0 || estado == RESERVADO) {
                //     let retorno = controlPresupuestal();
                //     context.currentRecord.setValue('custbody_lh_cp_estado_ppto_oc', EJECUTADO);
                //     if (runtime.getCurrentUser().id == DEVELOPER || runtime.getCurrentUser().id == DEVELOPERJP) {
                //         console.log('Pruebas internas');
    ...
    const controlPresupuestal = (typeMode = 0) => {
        const objRecord = currentRecord.get();
        let json = new Array();
        let json2 = new Array();
        let json3 = new Array();
            ...
            if (config != 0) {
                ...
                for (let i = 0; i < vueltas; i++) {
                    ...
                    if (transaction == TRANSACTION_JOURNAL_ENTRY) {//?JOURNAL
                        ...
                        if (objRecord.getSublistText({ sublistId: subList, fieldId: articulo, line: i }).length != 0) {
                            art = objRecord.getSublistText({ sublistId: subList, fieldId: articulo, line: i });
                        }
                ...
                console.log('arrayCecos', arrayCecos);
                for (let j in arrayCecos) {
                    resta = 0;
                    let ceco = arrayCategorias[j];
                    // existe_ppto = 'Tiene presupuesto';
                    let getCategoria = getDisponible(arrayCecos[j], year, month, config.temporalidad, ceco); //! getDisponible (FUNCTION)
                    let reservado = getReservado(arrayCecos[j], year, month, config.temporalidad); //! getReservado (FUNCTION)
    ...
    const verifyChanges = (context, estado) => {
        let scapeValidate = 0;
        let objSearch = '';
        let arrayAjuste = new Array();

        try {
            let retorno = true;
            let transaction = context.currentRecord.getValue({ fieldId: 'ntype' });
            if (transaction == TRANSACTION_EXPENSE_REPORT) {
                objSearch = search.load({ id: ER_EXPENSE_LINES_NO_GROUP_SEARCH });
            } else {
                objSearch = search.load({ id: PO_ITEM_LINES_NO_GROUP_SEARCH });
            }
            ...
            // if (searchResultCount != numLines && estado == RESERVADO || searchResultCount != numLines && estado == COMPROMETIDO) {
                ...
                for (let i in result) {
                    ...
                    if (context.currentRecord.type == PURCHASE_ORDER) {
                        if (amount < parseFloat(thisAmount)) {
                            console.log('Cambio por monto');
                            scapeValidate = 1;
                            // if (estado == EJECUTADO || estado == PAGADO) {
                            //     let thisCategoryPPTOFlag = context.currentRecord.getSublistValue({ sublistId: subList, fieldId: 'custcol_lh_ppto_flag', line: i });
                            //     arrayAjuste.push([thisCategoryPPTOFlag.toString(), parseFloat(thisAmount)]);
                            //     scapeValidate = 3;
                            // }
                            break;
                        }
                        ...
                        //!Test Success
                        if (amount > parseFloat(thisAmount)) {
                            console.log('Ajuste por monto');
                            let resta = amount - parseFloat(thisAmount);
                            // if (estado == EJECUTADO || estado == PAGADO) {
                            //     let thisCategoryPPTOFlag = context.currentRecord.getSublistValue({ sublistId: subList, fieldId: 'custcol_lh_ppto_flag', line: i });
                            //     arrayAjuste.push([thisCategoryPPTOFlag.toString(), parseFloat(thisAmount)]);
                            //     scapeValidate = 4;
                            // } else {
                            //     let thisCategoryPPTOFlag = context.currentRecord.getSublistValue({ sublistId: subList, fieldId: 'custcol_lh_ppto_flag', line: i });
                            //     arrayAjuste.push([thisCategoryPPTOFlag.toString(), parseFloat(resta)]);
                            //     scapeValidate = 2;
                            // }
                            let thisCategoryPPTOFlag = context.currentRecord.getSublistValue({ sublistId: subList, fieldId: 'custcol_lh_ppto_flag', line: i });
                            arrayAjuste.push([thisCategoryPPTOFlag.toString(), parseFloat(resta)]);
            ...
            if (estado == RESERVADO && scapeValidate == 2) {
                scapeValidate = 1;
            }
            ...
            } else if (scapeValidate == 0 && estado != COMPROMETIDO) {
                if (estado != RESERVADO) {
                    context.currentRecord.setValue('approvalstatus', 2);
                }
            } else if (scapeValidate == 2) {
                context.currentRecord.setValue('custbody_lh_categories_id_edit_flag', JSON.stringify(arrayAjuste));
            }
        ...
        try {
            ...
            for (let j in arrayCategorias) {
                ...
                if (json4[j].paramConfig != paramConfig2 || monto != parseFloat(thisArrayCategorias[j][1])) {
                    console.log('Cambio por parámetro según configuración es diferente o montos diferentes');
                    scapeValidate = 1;
                    break;
                }
            }
            ...
            if (scapeValidate == 1) {
                ...
                for (let j in arrayCategorias) {
                    ...
                    let getCategoria = getDisponible(arrayCategorias[j], year, month, temporalidad, ceco); //! getDisponible (FUNCTION)
